...
<?= Form::open(
        array(
            'route' => isset($article) ? array('admin.article.update', $article->id) : 'admin.article.store',
            'class' => 'saisie',
            'id' => 'article',
    ...
    <fieldset class="bloc_left">
        ...
        <p class="oblig<?= $errors->has('categorie_id') ? ' form_erreur' : '' ?>">
        ...
        <p class="<?= $errors->has('extrait') ? ' form_erreur' : '' ?> champ_max">
            ...
            <?= Form::textarea('extrait', isset($article) ? $article->extrait : null, array('rows' => 5)) ?>
        </p>
        <div class="<?= $errors->has('texte_md') ? ' form_erreur' : '' ?> p champ_max">
            ...
            <?= Form::textarea('texte_md', isset($article) ?  $article->texte_md : null, array('class' => 'markItUp', 'rows' => 15)) ?>
        </div>
        <p>
            <label for="submit">&nbsp;</label>
            <?= Form::submit('Enregistrer', array('id' => 'article_submit', 'class' => 'submit')) ?>
        </p>
    </fieldset>
<?= Form::close() ?>
...
<div class="bloc_right">
    ...
    <div id="medias">
    <?php if (isset($article) and $article->medias->count()) : ?>
        <?php foreach ($article->medias as $media) : ?>
        <div class="media<?= $article->media_id == $media->id ? ' media-illustration' : '' ?>">
            <a class="markItUpAddMedia media-image" href="<?= asset($media->path) ?>" title="<?= e($media->titre) ?>" data-fichier="<?= e($media->repertoire) ?>/<?= e($media->fichier) ?>">
                <?php if ($media->isImage()) : ?>
                <img src="<?= Croppa::url('/'.$media->cropPath, 150, 150) ?>" alt="" />
                <?php else : ?>
                <img src="<?= asset('admin/img/'.$media->icone) ?>" alt="<?= e($media->type) ?>" />
                <?php endif ?>
            </a>
            <div class="media-details">
                <div class="media-menu">
                    ...
                    <?php if ($media->isImage() and $article->media_id != $media->id) : ?>
                    <?= Form::open(array('method' => 'PUT', 'route' => array('admin.article.illustrer', $article->id))) ?>
                        <div>
                    ...
                    <?= Form::open(array('method' => 'DELETE', 'route' => array('admin.media.destroy', $media->id))) ?>
                        <div>
                            <input title="Supprimer le média" type="image" src="<?= asset('packages/ipsum/admin/img/supprimer.png') ?>" value="Supprimer" class="supprimer" data-message="<?= e($media->titre) ?>">
                        </div>
                    <?= Form::close() ?>
                </div>
                <?php if ($media->isImage()) : ?>
                ...
                <p>
                    <a class="markItUpAddMedia"
                        ...
                        title="<?= e($media->titre) ?>"
                        data-fichier="<?= e($media->repertoire) ?>/<?= e($media->fichier) ?>">
                            Ajouter <?= e($media->titre) ?>
                    </a>
                </p>
            </div>
       </div>
    ...
    <script id="tmpl-media" type="x-tmpl-mustache">
        {{#medias}}
            <div class="media">
            <a class="markItUpAddMedia media-image" href="{{{url}}}" title="{{{titre}}}" data-fichier="{{{repertoire}}}/{{{fichier}}}">
                {{#image}}
                <img src="{{{image}}}" alt="{{{titre}}}" />
                {{/image}}
                {{^image}}
                <img src="<?= asset('admin/img') ?>/{{{icone}}}" alt="" />
                {{/image}}
            </a>
            <div class="media-details">
                {{=|| ||=}}
                <div class="media-menu">
                    ...
                    <?= Form::open(array('method' => 'DELETE', 'route' => array('admin.media.destroy', '||id||'))) ?>
                        <div>
                            <input title="Supprimer le média" type="image" src="<?= asset('packages/ipsum/admin/img/supprimer.png') ?>" value="Supprimer" class="supprimer" data-message="'||titre||'">
                        </div>
                    <?= Form::close() ?>
                </div>
                ||={{ }}=||
                ...
                <p>
                    ...
                    <select name="alignement" class="alignement">
                        <option value="">---</option>
                        <option value="txtcenter">Centré</option>
                        <option value="left">Gauche</option>
                        <option value="right">Droite</option>
                    </select>
                </p>
                ...
                <p>
                    <a class="markItUpAddMedia"
                        href="{{{url}}}"
                        title="{{{titre}}}"
                        data-fichier="{{{repertoire}}}/{{{fichier}}}">
                            Ajouter {{{titre}}}
                    </a>
                </p>
            </div>
